...
export default function Sidebar() {
    ...
    function getSection(type: DutyType) {
        const sectionPosts = filteredPosts
            ...
            <section className={styles.section} key={type}>
                ...
                <ul className={styles.list}>
                    {sectionPosts.map((post) => {
                        const { title, type, difficulty } = post.meta
                        return <Item name={title} slug={post.slug} difficulty={difficulty} type={type} key={post.slug} />
                    })}
                </ul>
            </section>
        )
